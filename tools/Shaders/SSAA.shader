		...
		float4 frag(v2f i) :COLOR
		{
			...
			*/
			...
			else
			{
				...
				float4 c0 = tex2D(_MainTex, i.uv_MainTex + fixed2(0.9, 1) / _Size);
				float4 c1 = tex2D(_MainTex, i.uv_MainTex + fixed2(-0.9, 1) / _Size);
				float4 c2 = tex2D(_MainTex, i.uv_MainTex + fixed2(0.9, -1) / _Size);
				float4 c3 = tex2D(_MainTex, i.uv_MainTex + fixed2(-0.9, -1) / _Size);
